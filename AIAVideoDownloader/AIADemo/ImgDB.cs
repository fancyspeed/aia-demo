...
namespace AIADemo
{
    class ImgDB
    {
        ...
        static SearchRequest BuildRequest(int ioffset, int icount, string squery)
        {
            ...
            request.Video = new VideoRequest();
            request.Video.Count = (uint)icount;
            request.Video.CountSpecified = true;
            request.Video.Offset = (uint)ioffset;
        ...
        private void downloadImgs(SearchResponse response, string squery)
        {
          ...
          //  UTF8Encoding utf8 = new UTF8Encoding();
           // squery = utf8.GetString(utf8.GetBytes(squery));
            ...
            try
            {
                ...
                writer.Close();
                //response.Video.Results[0].StaticThumbnail.Url;

                // Download the images
                int i = 0;
                foreach (VideoResult result in response.Video.Results)
                {
                    i++;
                    img = getPhoto(result.StaticThumbnail.Url);
                    if (img != null)
        ...
        static void DisplayResponse(SearchResponse response, StreamWriter writer)
        {
            ...
            Console.WriteLine("Video results for " + response.Query.SearchTerms);
            Console.WriteLine(
                "Displaying {0} to {1} of {2} results",
                response.Video.Offset + 1,
                response.Video.Offset + response.Video.Results.Length,
                response.Video.Total);
            Console.WriteLine();

            // Display the Video results.
            foreach (VideoResult result in response.Video.Results)
            {
                writer.WriteLine(result.ClickThroughPageUrl);
                writer.WriteLine(result.PlayUrl);
                writer.WriteLine(result.RunTime);
                writer.WriteLine(result.SourceTitle);
                writer.WriteLine(result.Title);
                writer.WriteLine(result.StaticThumbnail.Url);
                writer.WriteLine(result.StaticThumbnail.Width);
                writer.WriteLine(result.StaticThumbnail.Height);
                writer.WriteLine();
                /*     Console.WriteLine(++i);
